...
(* he originally named this `set_cat` *)
definition
...
definition set_cat_UNIV :: "('c set, 'c set_arrow) Category"
  where "set_cat_UNIV \<equiv> MakeCat (set_cat' UNIV)"

definition set_cat :: "'c set \<Rightarrow> ('c set, 'c set_arrow) Category"
  where "set_cat U \<equiv> MakeCat (set_cat' U)"

subsection {* Simple Rules and Lemmas *}

lemma set_objectI [intro]:
  assumes "A \<subseteq> U"
  shows "A \<in> Obj (set_cat U)"
  using assms by (simp add: set_cat_def set_cat'_def MakeCat_def)

record ('d, 'a) Graph =
  dots :: "'d set"
  arrows :: "'a set"
  source :: "'a \<Rightarrow> 'd" (* or should be set_arrow? *)
  target :: "'a \<Rightarrow> 'd" (* OR plain associative list? *)

definition Graph_dot_D :: "('d, 'a) Graph" where
  "Graph_dot_D \<equiv> \<lparr> dots = (insert d {}), arrows = {},
source = undefined, target = undefined \<rparr>"
