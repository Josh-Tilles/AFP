...
lemma ZFfunDom [simp]: "|dom| (ZFfun A B f) = A"
by (auto simp add: ZFfun_def ZFfunDom_def Fst)

lemma ZFfunCod [simp]: "|cod| (ZFfun A B f) = B"
...
proof-
  ...
  have "ZFfun ( |dom| f) ( |cod| f) (\<lambda> x . f |@| x) = ?g |o| f" using a 
    by (simp add: ZFfun_ext ZFfunApp ZFfunCompApp SETfun)
...
(*
record Set_With_Endomorphism = 
  Carrier :: ZF ("carrier\<index>" 70)
  Action :: ZF ("action\<index>" 70)

locale SWE_Category = LSCategory +
  assumes "\<lbrakk>f maps X to Y\<rbrakk> \<Longrightarrow> (Action X) |o| f = f |o| (Action Y)"
